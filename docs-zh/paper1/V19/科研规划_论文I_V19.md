...
| 时间 | 论文 I | 论文 II | 学位 |
|------|--------|---------|------|
| 2026-06 | 数据 + franka 底座 | — | — |
| 2026-08 | 主实验数字 | — | — |
| 2026-09 | 投稿 RA-L/RCIM | — | — |
| 2027-03 | — | LangGraph 多 Agent + KG | — |
...
| 仓库 | 路径 | 职责 |
|------|------|------|
| **doctor/paper1/**（本仓） | `<franka_ros2>/doctor/paper1` | 数据集、Edge-IQA、LangGraph、离线实验、LaTeX、投稿包 |
| **franka_ros2 包** | 同仓 `franka_*` | ROS 2 Jazzy、MoveIt fake HW、Gazebo、FastAPI 边网关、pose Skills |

**路径**：`export PAPER1_ROOT="${FRANKA_ROS2_ROOT:-/root/work/franka_ros2}/doctor/paper1"`
...
```
┌──────────── Device ────────────┐     ┌──────────── Edge ────────────┐     ┌──── Cloud ────┐
│  Camera ──► 图像流              │     │  FastAPI ◄──► edge_iqa       │     │ Vision Stub   │
│  FR3 MoveIt ◄── LangGraph      │────►│  LangGraph 路由决策           │────►│ (upload)      │
└───────────────────────────────┘     └──────────────────────────────┘     └───────────────┘
         ▲                                      │
         └──────── resample / motion API ───────┘
```
...
| 数据流 | 说明 |
|--------|------|
| Camera → API | 采集图像进入边侧网关 |
| API → IQA | 边侧质量评分 |
| LangGraph → API / ARM | 路由与重拍控制 |
| LangGraph → Cloud | `upload_cloud` 分支 |
...
| ID | 任务 | 验收 |
|----|------|------|
| F0-01 | `doctor/paper1` 在本仓 | `PAPER1_ROOT` 可读 |
