...
| 指标 | 旧版（随机 uplift） | 物理重采 quick |
|------|---------------------|----------------|
| B2 M2 | 1.000 | **≈0.625**（80 帧） |
| 可审计性 | ❌ | ✅ JSONL 可记录 `skill_invoked` |
...
|----------|--------|-----|------|
| B2 | 208 ms | 0.625 | **主 claim** |
| B4 | 281 ms | 0.95 | Hybrid 对照 |
| B3 | 323 ms | 0.95 | 学习 IQA |
...
> 以上为 **1 seed 小样本趋势**；论文定稿需 `paper1_run_tcm_full.sh` 全量 3 seeds。
...
| 为什么 M2 不再是 1.0？ | 去掉随机 Q uplift，改用物理 Skill 变换；M2≈0.62–0.65 更诚实。 |
| B2m 和 B2u 都 0.65，选哪个？ | B2m 偏感知稳定（多帧）；B2u 偏决策优化（低 M3）。可按场景选，论文可并列报告。 |
| B2d M2 更低还值得写吗？ | 值得。创新点是**网络自适应**，需配合 RTT/丢包曲线，而非单看 M2。 |
| B4 比 B2 强为什么不用 B4 作主 claim？ | B4 依赖 MobileNet，牺牲可解释性与边侧 <30 ms 约束；B2 更适合医疗审计与 FR3 边侧网关。 |
...
6. **全量 3 seeds**：替换 quick 数字写入 Table II/III  
