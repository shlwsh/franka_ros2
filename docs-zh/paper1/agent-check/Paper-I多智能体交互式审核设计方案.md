...
> **可移植技能包（权威副本）**: `.cursor/skills/paper1-multi-agent-review/`（含 SKILL、设计方案、角色手册、预检清单等，**可复制到其他项目**）  
> **本文档**: 与技能包内 [设计方案.md](../../.cursor/skills/paper1-multi-agent-review/设计方案.md) 同步维护  
> **论文根目录**: `doctor/paper1/`（`PAPER1_ROOT`，见技能包 `config.md`）  
> **角色定义参考**: [多智能体角色定义手册.md](./多智能体角色定义手册.md)  
> **已知问题种子**: [Paper-I多智能体迭代完善方案.md](./Paper-I多智能体迭代完善方案.md) §1.3（C1–N4 等）
...
| 产出位置 | `doctor/paper1/reviews/{run_id}/` | **相同** |
...
5. **落盘交付**：在 `doctor/paper1/reviews/{run_id}/` 写入完整报告；并在对话中给出 **执行摘要 + 指向主报告路径**。
...
[ ] 记录 git rev（可选）: git -C doctor/paper1 rev-parse --short HEAD
[ ] 写入 00-上下文清单.md
```
...
3. 对每个 `[未共识]` 议题明确倾向性建议。  
4. 列出 **值得保留的亮点**（至少 3 条）。  
5. 判断：是否建议进入 [Paper-I多智能体迭代完善方案](./Paper-I多智能体迭代完善方案.md) 的 Phase B–E。
...
| # | 条件 |
|---|------|
| 1 | `reviews/{run_id}/` 下 11 个文件均已写入 |
| 2 | `05-全面评审报告.md` 含 §5 综合问题清单，且 P0 项 ≤ 7 条（过多则合并） |
...
| 用户后续指令 | 执行 Agent 行为 |
|--------------|-----------------|
| 「按行动清单修改论文」 | 转入 [Paper-I多智能体迭代完善方案.md](./Paper-I多智能体迭代完善方案.md)；扮演 📚+✍️ 改 tex |
...
若日后 Paper Forge / `pp-*` 上线，可将本手册 **编译为** 各角色的 `systemPrompt` 与检查清单；**当前权威执行路径仍是 Cursor 执行 Agent 按本文档手工模拟**。

工程参考（**不要求为本方案前置条件**）：

- [论文工坊技术实现方案.md](./论文工坊技术实现方案.md)  
- [论文上传评审与合并输出方案.md](./论文上传评审与合并输出方案.md)
...
| ID | 优先级 | 建议负责人 | 状态 | 关联 | 描述 | 位置 |
|----|--------|------------|------|------|------|------|
| AP-001 | P0 | 作者 | [ ] | C3 / STAT-1 | 统一 Abstract 与 Table II 的 M1 p50 | `00_abstract.tex` |
...
- **最优先 3 件事**: 1) … 2) … 3) …
- **主要亮点**: …
- **完整报告**: `doctor/paper1/reviews/{run_id}/05-全面评审报告.md`
```
...
| 文档 | 用途 |
|------|------|
| [多智能体角色定义手册.md](./多智能体角色定义手册.md) | 角色人格与职责原文 |
| [Paper-I多智能体迭代完善方案.md](./Paper-I多智能体迭代完善方案.md) | C1–N4 与修改 Phase B–F |
| [索引说明.md](./索引说明.md) | 本目录导航 |
| [../V19/论文1_V19.md](../V19/论文1_V19.md) | 论文科学问题与贡献表述 |
