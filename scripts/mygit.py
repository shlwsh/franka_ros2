...
def get_wsl_proxy():
    """自动检测 WSL 中的宿主机代理"""
    try:
        # 获取宿主机 IP (WSL2 中 nameserver 通常是宿主机 IP)
...
def main():
    ...
    if proxy_url:
        ...
        # 如果没检测到宿主机代理，则清理可能无效的 127.0.0.1 代理配置
        for var in ["http_proxy", "https_proxy", "HTTP_PROXY", "HTTPS_PROXY", "all_proxy", "ALL_PROXY"]:
    ...
    if not os.path.exists(env_file):
        ...
        print("请确保项目根目录下存在该文件，包含 DASHSCOPE_API_KEY, DASHSCOPE_BASE_URL, DASHSCOPE_MODEL")
        sys.exit(1)
    ...
    try:
        ...
        if diff_content and len(diff_content) > 15000:
            diff_content = diff_content[:15000] + "... (Diff truncated)"
            ...
            "messages": [
                {
                    ...
                    "content": "你是一个专业的 Git 提交信息生成助手。请根据代码变更生成简洁、清晰的中文提交信息。\n"
                               "规范要求：\n"
                               "1. 第一行为简短标题（不超过50字符），必须使用 Conventional Commits 前缀（feat, fix, docs, style, refactor, test, chore等）。\n"
                               "2. 使用中文，描述准确。\n"
                               "3. 不要带多余的引号、Markdown 代码块标记或额外解释。"
                },
